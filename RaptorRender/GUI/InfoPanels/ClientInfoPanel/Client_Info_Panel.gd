...
func update_client_info_panel(client_id : int):
	...
	var selected_client : Dictionary = RaptorRender.rr_data.clients[client_id]
	var props : Dictionary = selected_client.machine_properties
	...
	cpu_text += props["cpu"][0] + "\n"
	
	if props["cpu"][2] < 2:
		cpu_text += String(props["cpu"][2]) + " Socket, "
	else:
		cpu_text += String(props["cpu"][2]) + " Sockets, "
		
	if props["cpu"][3] < 2:
		cpu_text += String(props["cpu"][2] * props["cpu"][3]) + " Core, "
	else:
		cpu_text += String(props["cpu"][2] * props["cpu"][3]) + " Cores, "
		
	if props["cpu"][4] < 2:
		cpu_text += String(props["cpu"][2] * props["cpu"][4]) + " Thread"
	else:
		cpu_text += String(props["cpu"][2] * props["cpu"][4]) + " Threads"
	...
	var size_in_gb : String =  String( float(props["memory"]) / 1024 / 1024 )
	...
	var drive_count = 0
	for drive in props["hard_drives"]:
	...
	for i in range(0, props["graphics"].size()):
		graphics_text += props["graphics"][i] + "\n"
	...
	if props["ip_addresses"].size() > 1:
		var text : String = "IP " + tr("CLIENT_DETAIL_11") + ": \n"
		for i in range(0, props["ip_addresses"].size()):
			text += " " + props["ip_addresses"][i].replace(".", " . ") + "\n"
		if text.ends_with("\n"):
			text = text.substr(0,text.length()-1)
		IPLabel.text = text
	elif props["ip_addresses"].size() == 1:
		IPLabel.text = "IP:  " + props["ip_addresses"][0].replace(".", " . ")
	else:
		IPLabel.text = ""
		
		
	if props["mac_addresses"].size() > 1:
		var text : String = "MAC " + tr("CLIENT_DETAIL_11") + ": \n"
		for i in range(0, props["mac_addresses"].size()):
			text += " " + props["mac_addresses"][i].to_upper().replace(":", " : ") + "\n"
		if text.ends_with("\n"):
			text = text.substr(0,text.length()-1)
		MACLabel.text = text
	elif props["mac_addresses"].size() == 1:
		MACLabel.text = "MAC:  " + props["mac_addresses"][0].to_upper().replace(":", " : ")
	else:
		MACLabel.text = ""
	...
	if props["platform"].size() == 2:
		PlatformLabel.text = props["platform"][0] + "  " + props["platform"][1]
	if props["platform"].size() == 3:
		PlatformLabel.text = props["platform"][0] + "  " + props["platform"][1] + "  " + props["platform"][2]
