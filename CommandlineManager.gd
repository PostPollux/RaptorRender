...
func _ready():
	...
	var current_cmd_line_output_timer : Timer = Timer.new()
	current_cmd_line_output_timer.name = "Command Line Output Timer"
	current_cmd_line_output_timer.wait_time = 1
	current_cmd_line_output_timer.connect("timeout",self,"_on_current_cmd_line_output_timer_timeout")
	var root_node : Node = get_tree().get_root().get_node("RaptorRender")
	if root_node != null:
		root_node.add_child(current_cmd_line_output_timer)
	current_cmd_line_output_timer.start()


func _on_current_cmd_line_output_timer_timeout():
	pass
